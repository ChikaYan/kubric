...
parser.set_defaults(save_state=True, frame_end=FRAME_END, frame_rate=30, width=512, height=512)
FLAGS = parser.parse_args()

FLAGS.objects_set = 'shapenet'
FLAGS.seed = 1
np.random.seed(FLAGS.seed)
FLAGS.job_dir = 'output/decompose/single_car/'
if STATIC:
...
if FLAGS.camera == 'rotate':
  ...
  for i in range(len(camera_waypoints)):
    camera_waypoints[i]['frame'] = int(i / (len(camera_waypoints)-1) * FRAME_END)
...
sun = core.DirectionalLight(name="sun",
                            ...
                            intensity=0.6, position=(-11.6608, 6.62799, 25.8232))
lamp_key = core.PointLight(name="lamp_key",
                              color=color.Color.from_name("white"), intensity=150,
...
if FLAGS.camera == 'rotate':
  for i, waypoint in enumerate(camera_waypoints):
    ...
    theta_change = (waypoint['rot'][0] - last_waypoint['rot'][0] ) / ((frame_end- frame_start) / change_rate)
    phi_change = (waypoint['rot'][1] - last_waypoint['rot'][1] ) / ((frame_end- frame_start) / change_rate)
...
obj = add_random_shapnet_object(rng=rng, asset_id="02958343_f6906412622f5853413113c385698bb2", scale=4, object_restitution=0, position=(-3,-5,1), quaternion=(1,1,0.15,0.15))
obj = add_random_shapnet_object(rng=rng, asset_id="02958343_2928f77d711bf46eaa69dfdc5532bb13", scale=4, object_restitution=0, position=(5,3,1), quaternion=(1,1,-0.5,-0.5))
obj = add_random_shapnet_object(rng=rng, asset_id="02958343_9752827fb7788c2d5c893a899536502e", scale=4, object_restitution=0, position=(6,-5,1), quaternion=(1,1,0.5,0.5))
...
if not STATIC:
  ...
  obj = add_random_shapnet_object(rng=rng, asset_id='02958343_d4d7d596cf08754e2dfac2620a0cf07b', scale=4, object_restitution=0.9, position=(-1,-5,4), init_velocity=(-1,3,-1), object_friction=0)
  # obj = add_random_shapnet_object(rng=rng, asset_id="02958343_1a56d596c77ad5936fa87a658faf1d26", scale=4, object_restitution=0.8, position=(4,11,4), init_velocity=(-1,-3,0), quaternion=(1,1,3,3), object_friction=0)
  # obj = add_random_shapnet_object(rng=rng, asset_id="02958343_5876e90c8f0b15e112ed57dd1bc82aa3", scale=4, object_restitution=0.9, position=(-12,12,1), init_velocity=(3,-3,-4), quaternion=(0.2,0.2,-0.67,-0.67), object_friction=0)
  simulator.run(frame_start=0, frame_end=100)
  obj.velocity = (1,-2,0)
...
animation, collisions = simulator.run(frame_start=100, frame_end=scene.frame_end+1)
